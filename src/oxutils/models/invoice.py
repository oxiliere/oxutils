...
from .base import TimestampMixin, UserTrackingMixin, UUIDPrimaryKeyMixin
...
class InvoiceMixin(UUIDPrimaryKeyMixin, TimestampMixin, UserTrackingMixin):
    ...
    invoice_number = models.CharField(
        _("invoice number"), max_length=50, unique=True, help_text=_("Unique invoice number")
    )
    ...
    subtotal = models.DecimalField(
        _("subtotal"), max_digits=10, decimal_places=2, help_text=_("Amount excluding taxes")
    )
    ...
    tax_amount = models.DecimalField(
        _("tax amount"), max_digits=10, decimal_places=2, default=0.00, help_text=_("Tax amount")
    )

    total = models.DecimalField(
        _("total"), max_digits=10, decimal_places=2, help_text=_("Total amount including tax")
    )
    ...
    # Dates
    issue_date = models.DateField(_("issue date"), help_text=_("Invoice issue date"))

    due_date = models.DateField(_("due date"), help_text=_("Payment due date"))

    paid_date = models.DateTimeField(
        _("payment date"), null=True, blank=True, help_text=_("Payment date and time")
    )

    # Billing period
    period_start = models.DateField(_("period start"), help_text=_("Billing period start date"))

    period_end = models.DateField(_("period end"), help_text=_("Billing period end date"))

    # Additional info
    description = models.TextField(
        _("description"), blank=True, help_text=_("Detailed invoice description")
    )

    notes = models.TextField(
        _("notes"), blank=True, help_text=_("Internal notes about the invoice")
    )
    ...
    def generate_invoice_number(self):
        ...
        last_invoice = (
            self.__class__.objects.filter(invoice_number__startswith=f"INV-{year}{month:02d}")
    ...
    def is_overdue(self):
        ...
        return self.status == InvoiceStatusEnum.PENDING and timezone.now().date() > self.due_date
...
class InvoiceItemMixin(UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    name = models.CharField(
        _("service name"), max_length=200, help_text=_("Name of the billed service or product")
    )

    description = models.TextField(
        _("description"), blank=True, help_text=_("Detailed service description")
    )
    ...
    unit_price = models.DecimalField(
        _("unit price"), max_digits=10, decimal_places=2, help_text=_("Unit price excluding taxes")
    )
    ...
    metadata = models.JSONField(
        _("metadata"), default=dict, blank=True, help_text=_("Additional data in JSON format")
    )
...
class RefundRequestMixin(UUIDPrimaryKeyMixin, TimestampMixin, UserTrackingMixin):
    ...
    description = models.TextField(
        _("description"), help_text=_("Detailed description of the refund request")
    )
    ...
    processed_date = models.DateTimeField(
        _("processing date"), null=True, blank=True, help_text=_("Refund processing date and time")
    )

    admin_notes = models.TextField(
        _("admin notes"), blank=True, help_text=_("Internal administrator notes")
    )
