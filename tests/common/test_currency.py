...
from oxutils.currency.models import AVAILABLES_CURRENCIES, Currency, CurrencyState
...
class TestCurrencyModel(TestCase):
    ...
    def test_create_valid_currency(self):
        ...
        currency = Currency.objects.create(code="USD", rate=Decimal("1.0000"), state=self.state)
    ...
    def test_currency_str_representation(self):
        ...
        currency = Currency.objects.create(code="EUR", rate=Decimal("0.8500"), state=self.state)
    ...
    def test_currency_invalid_code(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            Currency.objects.create(code="INVALID", rate=Decimal("1.0"), state=self.state)
    ...
    def test_currency_invalid_rate_zero(self):
        ...
        with pytest.raises(ValueError) as exc_info:
            Currency.objects.create(code="USD", rate=Decimal("0"), state=self.state)
